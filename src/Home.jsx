    ...
    <div className=' bg-gray-100 w-screen h-screen'>
        <div className=' flex flex-col w-screen h-screen justify-center items-center'>
            ...
            <div className=' flex gap-3'>
                ...
                <button  className=' bg-blue-600 text-white px-4 rounded-md'>{isEdit ? <button onClick={updateTask}>Editar</button> : <button onClick={handleAddTask}>Adicionar</button>}</button>
            </div>

            <div className=' flex text-sm w-80 justify-evenly mt-4'>
                <p onClick={() => handleTabs(1)} className={`${tab === 1 ? 'text-blue-700' : 'text-black'} cursor-pointer`}>Todas</p>
                <p onClick={() => handleTabs(2)} className={`${tab === 2 ? 'text-blue-700' : 'text-black'} cursor-pointer`}>Ativas</p>
                <p onClick={() => handleTabs(3)} className={`${tab === 3 ? 'text-blue-700' : 'text-black'} cursor-pointer`}>Completas</p>
            </div>
                ...
                tab == 1 && todos?.map(todo => (
                    <div className=' flex justify-between bg-white p-3 w-80 mt-3 rounded-md'>
                        ...
                        <div className=' flex flex-col text-sm justify-start items-start'>
                            {todo.status == 'completed' ? null : <button className=' text-blue-600 cursor-pointer' onClick={() => handleEdit(todo.id, todo.task)}>Editar</button>}
                            <button className=' text-red-500 cursor-pointer' onClick={() => handleDelete(todo.id)}>Deletar</button>
                            {todo.status == 'completed' ? null : <button className=' text-green-600 cursor-pointer' onClick={() => handleComplete(todo.id)}>Completas</button>}
